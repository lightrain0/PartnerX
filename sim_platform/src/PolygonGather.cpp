...
#include <sim_platform/areaAction.h>
...
private:
  ...
  actionlib::SimpleActionServer<sim_platform::areaAction> as_;
  sim_platform::areaFeedback feedback_;
  sim_platform::areaResult result_;
  bool      gather_start_;
  /**
   ...
   */
   bool pointUpdateCb()
   {
     ...
     if(gather_start_ && 0 == GetPoseOnMap(newPose,frame_map_,frame_robot_))
     {
            ...
            && pointsNearby(input_.polygon.points.front(), newPoint, average_distance ))
        {
          ...
          //   while(nh_.ok())
             {
             //  polygon_planner_pub.publish(input_); // If we notify plan_path, uncomment this line 
               ros::spinOnce();
             }
   ...
   /**
   * @brief executing callback .
   * @param NULL
   */
   void executeCB(const sim_platform::areaGoalConstPtr &goal)
   {
      ROS_INFO("Area action server executing , goal position (%.2f,%.2f)",goal->origin.point.x,goal->origin.point.y);
      ...
      while(success == false)
      {
          ...
          success = pointUpdateCb();
          if(!success) // input_ will clear if success
            feedback_.area = input_;
          feedback_.area.header.stamp = ros::Time::now();
          as_.publishFeedback(feedback_);
          if(!ros::ok() || as_.isPreemptRequested())
           {
              ROS_INFO("Goal has been preempted");
              break;
           }
      }
...
public:
   ...
   */
  PolygonGather() :
      nh_(), private_nh_("~"),gather_start_(false),
      as_(nh_, "server_find_area", boost::bind(&PolygonGather::executeCB, this, _1), false)
  {
    frame_map_ = "odom";
