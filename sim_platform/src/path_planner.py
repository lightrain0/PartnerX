...
from sim_platform.msg import pathAction,pathGoal,pathResult,pathFeedback
...
class PathPlannerNode(object):
    ...
    def __init__(self):
        ...
        self.timeout = False
        self._sas = SimpleActionServer("server_plan_path",pathAction,execute_cb=self.execute_cb,auto_start=False)
        self._sas.start()
    ...
    def execute_cb(self,goal_msg):
        ...
        rospy.loginfo("receive from client")
        temp_points = []
        for point in goal_msg.area.polygon.points:
            temp_points.append( (float(point.x), float(point.y)) )
        self.field_shape = geo.Polygon(temp_points)
        self.field_frame_id = goal_msg.area.header.frame_id

        self.plan_path(self.field_shape, origin, 0)
    ...
    def field_callback(self, msg):
        ...
        rospy.loginfo("Handle the message of polygon")
